...
public class PatientController {
	...
	public Patient savePatient(@RequestBody PatientRegisteration pr) {
			Login l = new Login(pr.getUser_name(),pr.getPassword(),"Patient");
			Login inserted = lservice.saveUser(l);
			if(inserted == null) {
				return null;
			}
			else {
				Patient p = new Patient(pr.getFirst_name(),pr.getLast_name(),pr.getMobile_number(),pr.getGender(),pr.getBlood_group(),pr.getDob(),pr.getAadhar_number(),inserted);
