...
import com.cdac.project.entities.Appointment;
...
public interface AppointmentRepository extends JpaRepository<Appointment, Integer>{
	...
	@Query("select a from Appointment a where doctor_id= :d and ((appointmentDate < :date1 OR status = 'cancelled') OR (appointmentDate = :date1 and status = 'completed') ) ")
	...
	@Query("select a from Appointment a where patient_id= :p and ((appointmentDate < :date1 OR status = 'cancelled') OR (appointmentDate = :date1 and status = 'completed') ) ")
